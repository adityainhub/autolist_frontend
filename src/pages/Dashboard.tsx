import { useState } from "react";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { Skeleton } from "@/components/ui/skeleton";
import Navigation from "@/components/Navigation";
import { Instagram, Loader2 } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { toast } from "sonner";
import { generateListing as apiGenerateListing } from "@/services/api";

interface InstagramPost {
  id: string;
  caption: string;
  media_type: string;
  media_url: string;
  permalink: string;
  timestamp: string;
  username?: string;
}

interface FacebookPost {
  id: string;
  message?: string;
  created_time?: string;
  full_picture?: string;
  attachments?: {
    data?: Array<{
      media_type?: string;
      media_url?: string;
      type?: string;
      url?: string;
      media?: {
        image?: { src?: string; height?: number; width?: number };
        source?: string;
      };
      subattachments?: {
        data?: Array<{
          media?: { image?: { src?: string }; source?: string };
          media_type?: string;
          url?: string;
        }>;
      };
    }>;
  };
}
const Dashboard = () => {
  const [posts, setPosts] = useState<InstagramPost[]>([]);
  const [instagramLoading, setInstagramLoading] = useState(false);
  const [facebookPosts, setFacebookPosts] = useState<FacebookPost[]>([]);
  const [facebookLoading, setFacebookLoading] = useState(false);
  const [generatingId, setGeneratingId] = useState<string | null>(null);
  const navigate = useNavigate();

  const fetchInstagramPosts = async () => {
    setInstagramLoading(true);
    try {
      const response = await fetch(
        "https://graph.instagram.com/me/media?fields=id,caption,media_type,media_url,permalink,timestamp&access_token=IGAAf4avVPanxBZAFRFZAUxRZA2x0c0tXWGJnVlpLb3k3c19jOXl6a2l3TGdTUWszWVV6MEpNdGJfTG85c1BYMkVuLUxxZAXpnZAkpHNXFuY19UWVFYdzNpWEpfR245ODRWaUNGbldiRmVaTldfSkVNRDVLb0VpSjB1V3MxbllJeXktbwZDZD"
      );
      const data = await response.json();
      
      if (data.data) {
        setPosts(data.data);
        toast.success(`Loaded ${data.data.length} posts successfully!`);
      } else {
        toast.error("Failed to fetch posts. Please check your access token.");
      }
    } catch (error) {
      toast.error("Error fetching Instagram posts");
      console.error(error);
    } finally {
      setInstagramLoading(false);
    }
  };

  const fetchFacebookPosts = async () => {
    setFacebookLoading(true);
    try {
      const response = await fetch(
        `https://graph.facebook.com/v21.0/924499937408044/posts?fields=full_picture,message,attachments{media,type,url,subattachments},created_time&access_token=EAAXyUkpyG4EBQM2ksNgRlqX4ubWPACRV02AyLFDt0HZAm9qttY1ZAcZCT7yeElnOFxHLit01cFGRXNZC5bEy06ErOJ1ZCRPD9ndtT7QbVgKlKQX1ivxUTwAzyPVTZAqZC7jSdWoTuafZBLKGy9gZAQTlZAmPAEGKDibGXOxs0Bh3cDaMuVhyPE0o9NLDtvck9uVLoKZAmOn`
      );
      const data = await response.json();

      if (data.data) {
        setFacebookPosts(data.data);
        toast.success(`Loaded ${data.data.length} Facebook posts successfully!`);
      } else {
        toast.error("Failed to fetch Facebook posts. Please check your access token.");
      }
    } catch (error) {
      toast.error("Error fetching Facebook posts");
      console.error(error);
    } finally {
      setFacebookLoading(false);
    }
  };

  const generateListing = async (post: InstagramPost) => {
    setGeneratingId(post.id);
    try {
      // Fetch the image from Instagram URL and convert to File
      let imageFile: File | undefined;
      if (post.media_url && post.media_type === "IMAGE") {
        try {
          const imageResponse = await fetch(post.media_url);
          const imageBlob = await imageResponse.blob();
          imageFile = new File([imageBlob], "instagram-post.jpg", {
            type: "image/jpeg",
          });
        } catch (imageError) {
          console.warn("Failed to fetch image:", imageError);
          // Continue without image
        }
      }

      const response = await apiGenerateListing({
        text_content: post.caption || "",
        images: imageFile ? [imageFile] : undefined,
      });

      if (!response.success) {
        throw new Error(response.error || "Failed to generate listing");
      }

      navigate("/listing-editor", { state: { post, listing: response.listing } });
      toast.success("Listing generated successfully!");
    } catch (error) {
      console.error("Error generating listing:", error);
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to generate listing. Please try again."
      );
      setGeneratingId(null);
    }
  };

  const generateListingForFacebook = async (post: FacebookPost) => {
    setGeneratingId(post.id);
    try {
      let imageFile: File | undefined;

      // Mirror Instagram logic: only fetch a single direct image URL.
      // Prefer full_picture (direct CDN link) and fall back to first attachment media.image.src.
      const attachment = post.attachments?.data?.[0];
      const directImageUrl = post.full_picture || attachment?.media?.image?.src || attachment?.media_url;
      const mediaType = (attachment?.media_type || attachment?.type || "").toLowerCase();

      if (directImageUrl && (mediaType.includes("photo") || mediaType === "image" || post.full_picture)) {
        try {
          const imageResponse = await fetch(directImageUrl);
          const imageBlob = await imageResponse.blob();
          imageFile = new File([imageBlob], "facebook-post.jpg", {
            type: imageBlob.type || "image/jpeg",
          });
        } catch (imageError) {
          console.warn("Failed to fetch Facebook image:", imageError);
        }
      }

      const response = await apiGenerateListing({
        text_content: post.message || "",
        images: imageFile ? [imageFile] : undefined,
      });

      if (!response.success) {
        throw new Error(response.error || "Failed to generate listing");
      }

      navigate("/listing-editor", { state: { post, listing: response.listing } });
      toast.success("Listing generated successfully!");
    } catch (error) {
      console.error("Error generating listing:", error);
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to generate listing. Please try again."
      );
      setGeneratingId(null);
    }
  };

  return (
    <div className="min-h-screen">
      <Navigation />
      
      <div className="pt-32 pb-20 px-6">
        <div className="container mx-auto max-w-7xl">
          <div className="text-center space-y-6 mb-12 animate-fade-in">
            <h1 className="text-5xl md:text-6xl font-bold">
              Your Social
              <br />
              <span className="italic font-light">Posts</span>
            </h1>
            <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
              Connect your Instagram and Facebook accounts and transform product posts into Amazon listings
            </p>
          </div>

          {/* If neither Instagram nor Facebook has posts, show two connect cards */}
          {posts.length === 0 && facebookPosts.length === 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card className="max-w-md mx-auto border-2 border-dashed animate-scale-in">
                <CardContent className="pt-12 pb-12 text-center space-y-6">
                  <div className="w-20 h-20 mx-auto bg-gradient-to-br from-accent/20 to-accent/10 rounded-full flex items-center justify-center">
                    <Instagram className="h-10 w-10 text-accent" />
                  </div>
                  <div className="space-y-2">
                    <h3 className="text-2xl font-bold">Connect Your Instagram</h3>
                    <p className="text-muted-foreground leading-relaxed">
                      Import your Instagram product posts to start creating optimized Amazon listings with AI
                    </p>
                  </div>
                  <Button
                    size="lg"
                    className="rounded-full shadow-lg hover:shadow-xl transition-all"
                    onClick={fetchInstagramPosts}
                    disabled={instagramLoading}
                  >
                    <Instagram className="mr-2 h-5 w-5" />
                    {instagramLoading ? "Loading..." : "Fetch My Posts"}
                  </Button>
                </CardContent>
              </Card>

              <Card className="max-w-md mx-auto border-2 border-dashed animate-scale-in">
                <CardContent className="pt-12 pb-12 text-center space-y-6">
                  <div className="w-20 h-20 mx-auto bg-gradient-to-br from-blue-500/20 to-blue-500/10 rounded-full flex items-center justify-center">
                    <div className="text-blue-600 font-bold text-2xl">f</div>
                  </div>
                  <div className="space-y-2">
                    <h3 className="text-2xl font-bold">Connect Your Facebook</h3>
                    <p className="text-muted-foreground leading-relaxed">
                      Import your Facebook product posts and generate listings the same way as Instagram
                    </p>
                  </div>
                  <Button
                    size="lg"
                    className="rounded-full shadow-lg hover:shadow-xl transition-all"
                    onClick={fetchFacebookPosts}
                    disabled={facebookLoading}
                  >
                    {facebookLoading ? (
                      <>
                        <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                        Loading...
                      </>
                    ) : (
                      "Fetch Facebook Posts"
                    )}
                  </Button>
                </CardContent>
              </Card>
            </div>
          ) : (
            <div className="space-y-8">
              {/* Instagram Section */}
              {posts.length > 0 && (
                <div>
                  <div className="flex justify-between items-center mb-4">
                    <p className="text-lg text-muted-foreground">{posts.length} Instagram posts loaded</p>
                    <div className="flex gap-2">
                      <Button variant="outline" onClick={fetchInstagramPosts} disabled={instagramLoading}>
                        {instagramLoading ? (
                          <>
                            <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                            Refreshing...
                          </>
                        ) : (
                          "Refresh Instagram"
                        )}
                      </Button>
                    </div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                    {posts.map((post, index) => (
                      <Card
                        key={post.id}
                        className="overflow-hidden border-none shadow-lg hover:shadow-xl transition-all duration-300 animate-scale-in group"
                        style={{ animationDelay: `${index * 0.05}s` }}
                      >
                        <div className="aspect-square relative overflow-hidden bg-muted">
                          {post.media_type === "VIDEO" ? (
                            <video src={post.media_url} className="w-full h-full object-cover" controls />
                          ) : (
                            <img
                              src={post.media_url}
                              alt={post.caption?.substring(0, 50) || "Instagram post"}
                              className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                            />
                          )}
                        </div>
                        <CardContent className="pt-4 pb-4 space-y-3">
                          <p className="text-sm text-muted-foreground line-clamp-3 leading-relaxed">
                            {post.caption || "No caption available"}
                          </p>
                          <Button
                            className="w-full rounded-full shadow hover:shadow-lg transition-all"
                            onClick={() => generateListing(post)}
                            disabled={generatingId === post.id}
                          >
                            {generatingId === post.id ? (
                              <>
                                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                Generating...
                              </>
                            ) : (
                              "Generate Listing"
                            )}
                          </Button>
                        </CardContent>
                      </Card>
                    ))}
                  </div>
                </div>
              )}

              {/* Facebook Section */}
              {facebookPosts.length > 0 && (
                <div>
                  <div className="flex justify-between items-center mb-4">
                    <p className="text-lg text-muted-foreground">{facebookPosts.length} Facebook posts loaded</p>
                    <div className="flex gap-2">
                      <Button variant="outline" onClick={fetchFacebookPosts} disabled={facebookLoading}>
                        {facebookLoading ? (
                          <>
                            <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                            Refreshing...
                          </>
                        ) : (
                          "Refresh Facebook"
                        )}
                      </Button>
                    </div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {facebookPosts.map((post, index) => {
                      const attachment = post.attachments?.data?.[0];
                      const mediaUrl =
                        post.full_picture ||
                        attachment?.media_url ||
                        attachment?.media?.image?.src ||
                        attachment?.media?.source ||
                        attachment?.url ||
                        attachment?.subattachments?.data?.[0]?.media?.image?.src ||
                        attachment?.subattachments?.data?.[0]?.media?.source;
                      const isVideo = (attachment?.media_type || attachment?.type)?.toLowerCase().includes("video");
                      return (
                        <Card
                          key={post.id}
                          className="overflow-hidden border-none shadow-lg hover:shadow-xl transition-all duration-300 animate-scale-in group"
                          style={{ animationDelay: `${index * 0.05}s` }}
                        >
                          <div className="aspect-square relative overflow-hidden bg-muted">
                            {isVideo && mediaUrl ? (
                              <video src={mediaUrl} className="w-full h-full object-cover" controls />
                            ) : mediaUrl ? (
                              <img src={mediaUrl} alt={post.message?.substring(0, 50) || "Facebook post"} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                            ) : (
                              <div className="w-full h-full flex items-center justify-center text-muted-foreground">No media</div>
                            )}
                          </div>
                          <CardContent className="pt-4 pb-4 space-y-3">
                            <p className="text-sm text-muted-foreground line-clamp-3 leading-relaxed">
                              {post.message || "No message available"}
                            </p>
                            <Button
                              className="w-full rounded-full shadow hover:shadow-lg transition-all"
                              onClick={() => generateListingForFacebook(post)}
                              disabled={generatingId === post.id}
                            >
                              {generatingId === post.id ? (
                                <>
                                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                  Generating...
                                </>
                              ) : (
                                "Generate Listing"
                              )}
                            </Button>
                          </CardContent>
                        </Card>
                      );
                    })}
                  </div>
                </div>
              )}
            </div>
          )}
        </div>
      </div>
    </div>
  );
};

export default Dashboard;
